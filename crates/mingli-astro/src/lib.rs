...
mod tests {
    ...
    fn our_new_moons_match_an_independent_transcription() {
        ...
        for k in -5000..=20_000_i64 {
            ...
            // 先自己拦住荒唐的值，别递给另一份实现。
            //
            // 下面要把 `ours` 转成日期再交给 `astro` 反算。它对合理日期收敛得很快，
            // 对荒唐日期就不一定了——取样拉到两万个朔之后，变异扫描里有十四个变异
            // 是这样**挂死**的：既没被拦也不算漏网，只是把测试停在那里。
            // 区间由取样范围本身定：k = −5000 到 20000 对应 JD 约 2.30e6 到 3.04e6。
            assert!(
                (2_250_000.0..3_100_000.0).contains(&ours),
                "第 {k} 个朔算出 JD {ours}，离取样区间该有的范围太远——朔的算式坏了"
            );
