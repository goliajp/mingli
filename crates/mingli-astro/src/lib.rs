...
mod tests {
    ...
    fn our_new_moons_match_an_independent_transcription() {
        ...
        // 容差按**实测**定，不按「看起来够小」定。
        //
        // 实测（2026-09-01）：621 个对上的朔里最大差 4.023e-6 分钟。此前这里写的是
        // 0.001，比实测宽 250 倍——那不是余量，是藏身处：变异扫描下 `new_moon_jd_ut`
        // 一个函数留了十七个漏网，全都小到钻得进这 250 倍里。
        //
        // 现在留 2.5 倍。它挡不住的只剩下真的小到看不见的高阶项（角度式里的 t³、t⁴），
        // 那些要把 k 拉到几千才显形，而那时另一侧的模型是否还跟得上是另一个问题。
        assert!(
            worst < 1e-5,
            "与另一份转写最大差 {worst:.3e} 分钟，实测记录是 4.023e-6——有一侧的系数动过了"
        );
    }
}
