...
impl CastingEngine for TibetanEngine {
    ...
    fn profile(&self) -> &'static [DetItem] {
        ...
        const { &[
            ...
            d("日 parkha 与历日 parkha（未实现）", Und, "🟡 这两项**不是定不下，是本轮没做**——四源一致可落 Det：《白琉璃》给月建起卦表（虎马狗月初一＝Li 等），Janson E.10 给公式 (D + 6A + 6) amod 8，Erlewine 与 tibastro 的月表与编号逐条相同，且 Janson 公式复算 Erlewine 的算例逐字吻合。落码前须注意月序→生肖的映射 Phugpa 与 Tsurphu 不同，公式要以生肖而非月号为参"),
