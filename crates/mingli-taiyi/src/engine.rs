...
impl CastingEngine for TaiyiEngine {
    ...
    fn reading_notes(&self) -> Option<&'static str> {
        Some("\n【字段语义提示（太乙神数，读懂后给出有据的周期位置判读）】\n\
            ...
            - 🟡 本盘只出二目一系；君臣民基 / 大游小游 / 四神 / 十精等其余诸神未收（见确定性谱），\
              起法已查到，但只见《太乙统宗宝鉴》一系，不够两个独立来源，所以不出。别当漏算。\n\
    ...
    fn profile(&self) -> &'static [DetItem] {
        ...
        const { &[
            ...
            d("君臣民基 / 大游小游 / 四神 / 十精等其余诸神", Und, "🟡 现只做了二目一系（文昌 / 始击 → 主客算 → 大将 / 参将），这一系两部原典明载且有纪年实例可校。其余诸神的起法**已查**（此前记作「没查」，现更正）：\
                 君基 / 臣基 / 民基 / 五福 / 天乙 / 地乙 / 值符 / 四神 / 大游 / 小游 十者，\
                 起法见《太乙統宗寶鑑》卷一 / 二 / 四 / 十 / 二十，kentang2017/kintaiyi 逐条实现并逐处注明卷次\
                 （如君基取 `(积年 + 250) mod 360 / 30`，自午起十二支；臣基 / 民基 / 四神 / 天乙等按局数 1..72 循环取表）。\
                 但**仍是单典**：《太乙金镜式经》自称「運式之儀有八」，无此十者——\
                 与「定计目」是同一处两书差异。一实现 + 一典不足两独立源，故仍不收；\
                 要收须另取一部与《統宗寶鑑》独立的本子对校"),
