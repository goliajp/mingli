//! 把全树排出来的盘打成一个指纹，供 `scripts/profile-parity.sh` 取三次：native debug、
//! native release、以及编到 wasm32 后在 node 里跑的那一份。
//!
//! 用 `cast_all_detailed` 而不是 `cast_all`，因为 wasm 那扇门导出的 `cast` 走的正是它——
//! 要比就得比同一个形状。
//!
...
fn main() {
    // 首行是输入清单本身。时刻表在这里与 `scripts/wasm-cast.mjs` 各写了一遍，
    // 谁改一边忘了另一边，比较器就会拿两组不同的输入互比而毫无察觉——
    // 把输入写进产物，让比较器第一件事就是核对它们。
    let mut blob = format!("{MOMENTS:?}\n");
    for (year, month, day, hour, minute) in MOMENTS {
        ...
        let all = cast_all_detailed(&registry(), &query);
    ...
    // 给了路径就把整份落盘，供 `scripts/wasm-parity.sh` 跟 wasm 那份逐字段比。
    if let Some(path) = std::env::args().nth(1) {
        std::fs::write(&path, &blob).expect("落盘");
    }
