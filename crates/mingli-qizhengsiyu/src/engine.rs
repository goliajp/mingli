...
impl CastingEngine for QizhengsiyuEngine {
    ...
    fn reading_notes(&self) -> Option<&'static str> {
        Some("\n【字段语义提示（七政四余，中国本土星占）】\n\
            - `stars[]`：日月五星 + 四余中的三余（罗㬋 / 计都 / 月孛；紫炁不出）。每颗带 `longitude` 黄经、\
              `sign` / `sign_name` 所落十二宫、`degree_in_sign` 宫内度数、\
              `is_qizheng` 标它是七政（真天体）还是四余（算点）。**各星所入之宿不在盘上**，别自己推。\
