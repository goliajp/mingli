...
/// 三套宿度表**逐宿**对原典，不是只对小计。
///
/// 先前只验四方小计与周天（5 个约束 / 28 个数），我并写过一句判词说逐条转录「成本大过风险」——
/// 那是替委托方做取舍，不该由我做。现在逐条对：
///
/// - **汉制**：《汉书·律历志》「角十二 亢九 氐十五 房五 心五 尾十八 箕十一 /
///   斗二十六又四分之一 牛八 女十二 虚十 危十七 室十六 壁九 / 奎十六 娄十二 胃十四 昴十一
///   毕十六 觜二 参九 / 井三十三 鬼四 柳十五 星七 张十八 翼十八 轸十七」，**28 宿逐条相符**。
///   唯一的差是斗：本表存 26、原典作 26¼——那 ¼ 度余分的归属本身是已声明的 Und
///   （《淮南子》挂箕、《汉书》挂斗），故基表存整数、余分由 `QUARTER_REMAINDER` 单列。
/// - **授时**：维基文库《元史·卷054》赤道宿度，**28 宿的整度部分逐条相符**，
///   四方小计 79.20 / 93.8075 / 83.85 / 108.40 精确相等（「九十三度八十分**太**」的「太」即 ¾ 分）。
///   分位的小数部分只被四方小计约束，未能另取到逐宿原文。
/// - **大衍**：逐宿原文未能取到（维基文库该卷 404、ctext 对应页已删）。能定的定死——
///   南方小计 112 → 111 且只有鬼变，**鬼 = 3 由小计唯一确定**；毕觜参三者只被
///   「西方 80 → 81」这一个约束绑住，**其和 = 28 可验，个体拆分不可验**，故本测试只验其和，
///   并在此写明这一处的证据到哪为止。
#[test]
fn every_lodge_table_matches_its_source_entry_by_entry() {
    use xiudu::{table, Epoch};
    // 《汉书·律历志》的二十八宿距度（万分之一度；斗的 ¼ 余分另由 QUARTER_REMAINDER 处理）
    const HAN_TEXT: [u32; 28] = [
        120_000, 90_000, 150_000, 50_000, 50_000, 180_000, 110_000,
        260_000, 80_000, 120_000, 100_000, 170_000, 160_000, 90_000,
        160_000, 120_000, 140_000, 110_000, 160_000, 20_000, 90_000,
        330_000, 40_000, 150_000, 70_000, 180_000, 180_000, 170_000,
    ];
    // 《元史·卷054》各宿的**整度**（分位未能另取原文，由四方小计约束）
    const SHOUSHI_WHOLE: [u32; 28] = [
        12, 9, 16, 5, 6, 19, 10,
        25, 7, 11, 8, 15, 17, 8,
        16, 11, 15, 11, 17, 0, 11,
        33, 2, 13, 6, 17, 18, 17,
    ];
    let names = xiudu::NAMES;

    let han = table(Epoch::Han);
    for (k, want) in HAN_TEXT.iter().enumerate() {
        assert_eq!(han[k], *want, "汉制 {} 宿：原典 {} 度，本表 {} 度", names[k], f64::from(*want) / 1e4, f64::from(han[k]) / 1e4);
    }

    let shoushi = table(Epoch::Shoushi);
    for (k, want) in SHOUSHI_WHOLE.iter().enumerate() {
        assert_eq!(shoushi[k] / 10_000, *want, "授时 {} 宿：《元史》作 {want} 度余，本表 {} 度", names[k], f64::from(shoushi[k]) / 1e4);
    }

    // 大衍：证据到「鬼由小计唯一确定 + 毕觜参之和」为止
    let dayan = table(Epoch::Dayan);
    assert_eq!(dayan[22], 30_000, "大衍 鬼宿由南方小计 112→111 唯一确定为 3 度");
    let bi_zi_shen: u32 = dayan[18] + dayan[19] + dayan[20];
    assert_eq!(bi_zi_shen, 280_000, "大衍 毕+觜+参 由西方小计 80→81 定为 28 度；个体拆分本测试验不了");
    // 其余二十四宿大衍同汉制——《新唐书》自称只动了那四宿
    for k in 0..28 {
        if [18, 19, 20, 22].contains(&k) {
            continue;
        }
        assert_eq!(dayan[k], han[k], "大衍 {} 宿本不该与汉制不同", names[k]);
    }
}
