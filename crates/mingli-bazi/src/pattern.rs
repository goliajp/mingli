...
///    年/月/时三干头是否「透出」，跳过比肩/劫财，先透先定；透干所对日主的十神 = 格局名。
/// 3. **暗格**：没有可取八正格的月令藏干透出 → 取本气暗藏立格。
///
/// 输出是本规则下的符号分类，不包含吉凶预测。用神另由旺衰与五行分布计算。
///
...
pub fn determine_pattern(
    ...
    for (i, &qi) in hidden.iter().enumerate() {
        // 中气或余气为比劫也不进入八正格；禄刃只由前面的本气分支处理。
        let god = ten_god(dm, qi);
        if matches!(god, "比肩" | "劫财") { continue; }
    ...
    // 没有可取八正格的透干（可能仅比劫透出）：取本气暗藏立格
    let god = ten_god(dm, main_qi);
    Pattern {
        name: format!("暗{god}格"),
        source: "月令本气暗藏（年/月/时三干头无可取八正格的透干）".to_string(),
        qi_stem: STEMS[main_qi as usize].to_string(),
        qi_kind: "本气".to_string(),
        revealed_in: None,
        ten_god: god.to_string(),
