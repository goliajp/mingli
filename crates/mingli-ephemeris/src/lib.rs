...
///
/// 这里的 `+ 180` 改成 `- 180` 不会有任何测试红，那不是守卫的缺口：模 360 之下
/// 两者恒等（`(x+180) mod 360 == (x−180) mod 360`）。变异扫描把它列成漏网，
/// 而它是**等价变异**——写在这里，省得下一个跑扫描的人再推一遍。
#[must_use]
...
#[cfg(feature = "vsop87")]
/// 换一套删节级数省不到东西——两条都量过（2026-08-30）。
///
/// 全量 VSOP87D 的常量表在 wasm 产物里约 780 KB，占一个带星历档位的九成，
/// 而算完那些级数是排一张盘里九成七的时间。顺理成章的想法是换 Meeus《Astronomical
/// Algorithms》附录 III 的删节级数——`astro` crate 就是它的 Rust 实现，本 crate 算月亮
/// 已经在用，源码里那份数据 31,953 行，对照 `vsop87` crate 的 820,691 行，少 26 倍。
///
/// 两件事都不成立：
///
/// | | 全量 VSOP87D | Meeus 删节 |
/// |---|---|---|
/// | 七行星一遍 | 268.17 µs | 261.81 µs（快 1.02×） |
/// | wasm `-Oz` 后 | 807,701 字节 | **1,619,286 字节（大一倍）** |
///
/// 精度倒是没问题：1900–2100 每两年取样，两者黄经最大差 0.004″。
///
/// 体积反过来的原因在存法而不在项数：`vsop87` 把系数存成静态常量数组，
/// `astro` 存成运行时构造 `Vec` 的代码，而代码比打包好的 f64 数组大。
/// 源码行数少 26 倍是误导。
///
/// 上面那句「快 1.02×」曾被我读成「截断不是速度杠杆」。那是错的：那次比的是
/// `astro` 的实现对 `vsop87` 的实现，把「项数」与「存法」混成了一个变量。
/// 同一实现、只换表才是干净的比较——八颗行星的 L/B/R 全阶，按振幅 1e-6 截断：
///
/// | | 项数 | 求值一遍 |
/// |---|---|---|
/// | 全量 | 24,158 | 1099.1 µs |
/// | 截断 1e-6 | 1,536（6.4%） | 83.3 µs（**快 13.2×**） |
///
/// 精度代价（日心黄经，1900–2100 每十年取样，对全量）：
///
/// | 阈值 | 保留 | 水星 | 地球 | 木星 | 土星 |
/// |---|---|---|---|---|---|
/// | 1e-8 | 10–55% | 0.016″ | 0.026″ | 0.037″ | 0.027″ |
/// | 1e-7 | 4–23% | 0.097″ | 0.138″ | 0.234″ | 0.221″ |
/// | 1e-6 | 1.7–10% | 1.150″ | 1.718″ | 1.039″ | 1.068″ |
/// | 1e-5 | 0.8–4% | 7.293″ | 5.723″ | 8.880″ | 9.042″ |
///
/// 也就是说 `eph-lite` 这一档已经被量清楚了：表缩约 15.7 倍（780 KB → 约 50 KB）、
/// 级数求值快约 13 倍、代价是 1.7 角秒——而公开盘只精确到角分（60″）。
/// 它也解释了 astronomy-engine 那三十倍从哪来。
///
/// 做法：按振幅截断 `vsop87` 自己那张表（MIT/Apache，与本项目兼容），
/// 同样存成常量数组。系数不是新的权威值，是已验证那份的子集，残差对着全量量。
/// 不在 v1 范围内，但已不再缺任何东西。
///
/// 每颗行星的光行时迭代轮数。
///
...
mod tests {
    ...
    /// Ω 与 Π 的高阶项，在拉开的历元上钉住。
    ///
    /// 上下这几条全都取在 J2000（T=0）或一个整周期之后——那正是 T²、T³、T⁴ 项一起消失
    /// 或转回原处的地方。于是这两个多项式的高阶系数无人过问：变异扫描下
    /// `mean_lunar_node` 与 `mean_lunar_perigee` 共留十四个漏网，改了整包照常绿。
    ///
    /// 这里取 T = ±1 与 ±20。T=±20 上 `T³/467441` 涨到 0.267°、`T⁴/60616000` 到 0.103°，
    /// 远在容差之上；而 T=±1 处它们只有 2e-6 与 1.6e-8 度，段内取样看不见。
    ///
    /// 期望值由两条注释里已经引用的公式逐项算出，钉的是转写不是新的权威值——
    /// 正确性由上面对 SOFA 与三源系数的比对守着。
    #[test]
    fn lunar_node_and_perigee_higher_order_terms_are_pinned() {
        for &(jde, want_node, want_perigee) in &[
            (1_721_045.0_f64, 288.580_735_874_f64, 59.059_027_790_f64), // T = −20
            (2_415_020.5, 259.156_433_305, 334.384_912_389),            // 1900，T = −1
            (2_451_545.0, 125.044_547_900, 83.353_246_500),             // 2000，T = 0
            (2_488_070.5, 350.883_859_497, 192.412_344_240),            // 2100，T = +1
            (3_182_045.0, 323.163_400_792, 99.408_308_202),             // T = +20
        ] {
            let got_node = mean_lunar_node(jde);
            assert!(
                (got_node - want_node).abs() < 1e-9,
                "Ω(JDE {jde}) = {got_node}，应为 {want_node}——系数动过了"
            );
            let got_perigee = mean_lunar_perigee(jde);
            assert!(
                (got_perigee - want_perigee).abs() < 1e-9,
                "Π(JDE {jde}) = {got_perigee}，应为 {want_perigee}——系数动过了"
            );
        }
    }
