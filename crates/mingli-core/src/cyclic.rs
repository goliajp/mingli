...
pub fn gcd(a: i64, b: i64) -> i64 {
    ...
    // 至多九十步：辗转相除每两步至少把较小数减半，i64 有六十四位。
    //
    // 从前这里是 `while b != 0`，靠取余收敛。把那个 `%` 改成 `/`，余数不再变小，
    // 循环就再也退不出去——变异扫描把它记成超时，而超时既不算拦住也不算漏网，
    // 只是把测试挂在那儿。上限本来就在，写出来即可。
    for _ in 0..90 {
        if b == 0 {
            return a;
        }
    ...
    unreachable!("辗转相除每两步至少减半，i64 上九十步内必到 0")
}
...
pub fn lcm(a: i64, b: i64) -> i64 {
    // 这个 `||` 改成 `&&` 是等价变异：任一为零时，另一支算出来也是零
    // （`(0 / gcd(0,b)) * b = 0`、`(a / gcd(a,0)) * 0 = 0`）。
    if a == 0 || b == 0 {
        0
    } else {
        (a / gcd(a, b)) * b
...
fn crt_pair(r1: i64, m1: i64, r2: i64, m2: i64) -> Option<(i64, i64)> {
    ...
    // 这一处确是等价变异。
    //
    // `l` 不是。从前这里写着「`l` 放大成任何倍数也无所谓」——**只在两个同余式时成立**。
    // 返回的 `l` 会当作下一轮的 `m1` 传下去，而 `crt_combine` 是公开的、接受任意多个：
    // `[(3,10), (5,12), (2,7)]` 原本得 233，把 `m1 / g * m2` 写成 `m1 * g * m2` 得 1493。
    // 本仓当前的调用方（干支、玛雅）都只合两个，所以这条错了很久也没显形。
    // 由 `crt_combine_chains_three_moduli` 钉住。
    let step = m2 / g;
...
mod tests {
    ...
    /// 三个模数连着合并——返回的模数要真是最小公倍数，因为它会传给下一轮。
    ///
    /// 本仓当前只合两个（干支 10/12、玛雅 13/20），而两个的时候 `crt_pair` 返回的模数
    /// 只用来给 x 取模，x 本来就小于它，放大多少都看不出来。第三个同余式一进来，
    /// 那个模数就成了下一轮的 `m1`，错了结果跟着错。
    ///
    /// 实测：`[(3,10), (5,12), (2,7)]` 应得 233；把 `m1 / g * m2` 写成 `m1 * g * m2`
    /// 会得 1493。期望值由中国剩余定理直接验算：233 mod 10 = 3、mod 12 = 5、mod 7 = 2，
    /// 且 233 < lcm(10,12,7) = 420。
    #[test]
    fn crt_combine_chains_three_moduli() {
        let x = crt_combine(&[(3, 10), (5, 12), (2, 7)]).expect("三式相容");
        assert_eq!(x, 233, "三个模数合并的结果");
        assert_eq!(x % 10, 3);
        assert_eq!(x % 12, 5);
        assert_eq!(x % 7, 2);
        assert!(x < 420, "结果应落在 lcm(10,12,7) = 420 之内，实得 {x}");

        // 两个的时候也顺带钉住，跟上面那条一起说明「差别只在第三个进来之后」。
        assert_eq!(crt_combine(&[(3, 10), (5, 12)]), Some(53));
        assert_eq!(53 % 10, 3);
        assert_eq!(53 % 12, 5);
    }
