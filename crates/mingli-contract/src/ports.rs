...
pub trait CastingEngine: Send + Sync {
    ...
    /// # 写法
    ///
    /// 读的人（语言模型）手上只有本叶输出的那份 JSON。它认得字，不认得这套系统，
    /// 于是缺什么就补什么——**缺的是字段与领域概念之间的那一层**，不是结论。
    ///
    /// 1. **一条一个字段**，用反引号写出**真实的 JSON 路径**（`gates.zhi_shi_gate`，
    ///    不是「值使门」）。路径写错，读的人会去找一个不存在的键
    /// 2. **说取值域**：几个值、什么含义、有没有序（「旺相休囚死，前二为强、后三为弱」）
    /// 3. **说结构事实，不下断语**。「三奇临吉门，传统视为得力」可以；「主大吉」不可以——
    ///    吉凶归释义后端在护栏内自己判，读法只交事实
    /// 4. **指明先看哪里**。每套系统都有起论之处（值符宫、命宫、法官位），
    ///    没有这一句，模型会把九个宫平铺着讲一遍
    /// 5. **点到 🟡**：本叶 [`profile`](CastingEngine::profile) 里标欠定的地方，
    ///    读法里也要说一声，免得模型把留白当成漏算
    /// 6. **篇幅与字段数相称**。字段少的叶两三句就够；把一片叶的读法写成一句话，
    ///    等于没写（守卫会红）
    ///
    /// 已有的三条可作样板：四柱（字段最多、层次最深）、紫微（宫位制）、奇门（四盘叠加）。
