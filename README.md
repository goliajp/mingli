...
> 38 crates · 24 leaves (21 time-driven leaves fan out in parallel, 4 word-driven leaves go through `/api/word`, one of them both) · 8 intents, over HTTP and wasm alike · 791 tests green
...
someone once did by hand into a command anyone can re-run: it plants 86 known faults
...
./scripts/guard-probe.sh   # plants 86 known faults, checks the guard that should catch each one does
```
