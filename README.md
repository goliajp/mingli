...
> 38 crates · 24 leaves (21 time-driven leaves fan out in parallel, 4 word-driven leaves go through `/api/word`, one of them both) · 8 intents, over HTTP and wasm alike · 796 tests green
...
It also runs each of the 38 crates' tests in isolation: `cargo test --workspace` compiles
...
someone once did by hand into a command anyone can re-run: it plants 101 known faults
...
```bash
cargo test --workspace     # 796 tests
...
./scripts/guard-probe.sh   # plants 101 known faults, checks the guard that should catch each one does
```
