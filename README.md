...
> 38 crates · 24 leaves (21 time-driven leaves fan out in parallel, 3 word-driven leaves go through `/api/word`) · 8 intents, over HTTP and wasm alike · 714 tests green
...
```bash
cargo test --workspace     # 714 tests
...
./scripts/api-snapshot.sh check snap.txt   # 36 requests, byte for byte
