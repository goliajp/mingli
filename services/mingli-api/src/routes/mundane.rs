...
use crate::dto::{ask_time, MundaneRequest};
...
/// 国运：算盘后交释义。
pub(crate) async fn interpret_handler(Json(req): Json<MundaneRequest>) -> Response {
    cast_then_interpret(
        || mingli_app::mundane::cast(leaves(), &ask_time(&req.founded_at), req.latitude, req.longitude, req.target_year, req.span),
        |json| {
            mingli_interpret::interpret_mundane(&crate::backend::ClaudeCli, &json)
