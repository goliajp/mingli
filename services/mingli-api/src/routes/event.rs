...
pub(crate) async fn handler(Json(req): Json<EventRequest>) -> Response {
    match mingli_app::event::cast(leaves(), &req.t_ask.ask_time(), req.seed, req.question) {
...
pub(crate) async fn interpret_handler(State(backend): State<Interpret>, Json(req): Json<EventRequest>) -> Response {
    cast_then_interpret(
        || mingli_app::event::cast(leaves(), &req.t_ask.ask_time(), req.seed, req.question),
        move |json| {
