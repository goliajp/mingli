...
pub(crate) async fn handler(Json(req): Json<LocativeRequest>) -> Response {
    match mingli_app::locative::cast(leaves(), &req.t_ask.ask_time(), req.seed, req.category) {
...
pub(crate) async fn interpret_handler(State(backend): State<Interpret>, Json(req): Json<LocativeRequest>) -> Response {
    cast_then_interpret(
        || mingli_app::locative::cast(leaves(), &req.t_ask.ask_time(), req.seed, req.category),
        move |json| {
