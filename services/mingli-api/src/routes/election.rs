...
pub(crate) async fn handler(Json(req): Json<ElectionRequest>) -> Response {
    match mingli_app::election::scan(&req.window_start.ask_time(), &req.window_end.ask_time(), req.category) {
...
pub(crate) async fn interpret_handler(State(backend): State<Interpret>, Json(req): Json<ElectionRequest>) -> Response {
    cast_then_interpret(
        || mingli_app::election::scan(&req.window_start.ask_time(), &req.window_end.ask_time(), req.category),
        move |json| {
