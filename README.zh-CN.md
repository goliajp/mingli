...
> 38 个 crate · 24 片叶（21 片时刻叶走并行 fan-out，4 片字词叶走 `/api/word`，其中一片两边都答）· 8 类问局（HTTP 与 wasm 都接）· 766 个测试全绿
...
「我当时手工试过」变成一条能重跑的命令：种 45 个已知的错，逐条问该拦它的守卫红没红。
...
./scripts/guard-probe.sh   # 种 45 个已知的错，看该拦它的那条守卫拦不拦
```
