...
> 39 个 crate · 24 片叶（21 片时刻叶走并行 fan-out，4 片字词叶走 `/api/word`，其中一片两边都答）· 8 类问局（HTTP 与 wasm 都接）· 811 个测试全绿
...
小到在本项目支持的年代里看不见的系数，以及遇到坏输入不报错、直接不返回的循环。
